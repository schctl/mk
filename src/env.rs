...
impl Env {
    ...
    pub fn init_args(&mut self) -> Result<(), NulError> {
    ...
    pub fn init_vars(&mut self) -> Result<(), NulError> {
    ...
    /// Push an argument.
    pub fn push_arg(&mut self, arg: &str) -> Result<(), NulError> {
        self.args.push(CString::new(arg)?);
        Ok(())
    }

    /// Push a key value pair to the environment variable list.
    pub fn push_var(&mut self, key: &str, val: &str) -> Result<(), NulError> {
